...
# СЂСџвЂќвЂ alias Р Т‘Р В»РЎРЏ middleware / services / dao
...
async def _exec_sql_file(path: Path):
    ...
    if not path.exists():
        ...
        logger.warning(f"РІС™В РїС‘РЏ SQL file not found: {path.name}")
...
async def init_db():
    ...
    logger.info("СЂСџвЂќР‹ DB bootstrap started")
    ...
    if SEED_SQL.exists():
        ...
        if not await _has_products():
            ...
            logger.info("СЂСџРЉВ± No products РІвЂ вЂ™ applying seed.sql")
    ...
    if ORDER_VIEW_SQL.exists():
        ...
        logger.info("СЂСџвЂњвЂћ Applying order_view.sql")
    ...
    logger.info("РІСљвЂ¦ DB bootstrap finished")
